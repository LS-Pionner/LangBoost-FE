<template>
    <div class="scrollable-container" @scroll="handleScroll">
      <div :class="{'sentenceset-container' : isAdmin}">
        <PublicSentenceSetComponent class="sentenceset-item"
...
import store from '@/store';
  ...
  const fetchSentenceSetList = async () => {
    ...
    try {
        const res = await instance.get(`api/v1/public/sentence-set?offset=${offset.value}`);
